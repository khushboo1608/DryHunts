          ...
          <div class="col-12">
            <div class="card">
              ...
              <div class="col-lg-12">
                    <div class="search_list">
                        <div class="add_btn_primary">
                            <a href="{{route('servicefile-export')}}">CSV Export</a> &nbsp;&nbsp;
                        </div>
                    </div>
                </div> 
                <div class="col-lg-12">
...
<script type="text/javascript">
        ...
        swal({
          ...
          if (willDelete) {
                ...
                $.ajax({
                    url: delurl,
                    type: "post",
                    data: {"_token": "{{ csrf_token() }}",'id': serviceId,'is_disable':status},
                    dataType: 'json',
                    success: function (data) {
                        if (data.result == true)
                        {
    ...
    function Featured(serviceId,featured)
    {
        ...
        swal({
            title: "Are you sure ?",
            text: "You want to "+btn_text+" this service Feature!",
            icon: "warning",
            buttons: true,
            dangerMode: true,
        }).then((willDelete) => {
          if (willDelete) {
                var delurl = "{{url('admin/service_featured')}}";
                $.ajax({
                    url: delurl,
                    type: "post",
                    data: {"_token": "{{ csrf_token() }}",'id': serviceId,'is_disable':featured},
                    dataType: 'json',
                    success: function (data) {
                        if (data.result == true)
                        {
                    ...
                    error: function (request, featured, error) {
                        if(request.featured == 419)
                        {
                            location.href = "{{url('admin/service')}}";
                        }
                    }
    ...
    $(".actions").click(function(e){
      ...
      if(_ids!='')
      {
        ...
        swal({title: 'Sorry no category selected!', type: 'info'});
      }
    });
